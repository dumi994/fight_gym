      ...
      <div class="form-row">
        ...
        <div class="form-group col-md-6">
         <div class="col-md-12">
            ...
            <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title" value="{{ old('title', $course->title ?? '') }}" required>
            @error('title')
          ...
          <div class="col-md-12 row">
            ...
            <div class="form-group col-md-4 col-sm-6">
              ...
              <input type="time" class="form-control @error('duration') is-invalid @enderror" name="duration" id="duration" value="{{ old('duration', $course->duration ?? '') }}" required>
              @error('duration')
              <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
              </span>
          ...
          <div class="col-md-12 row">
            ...
            <div class="form-group col-md-4 col-sm-6">
              ...
              <input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror" name="price" id="price" value="{{ old('price', $course->price ?? '') }}">
              @error('price')
              <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
              </span>
        ...
        <div class="form-group col-md-12">
          ...
          <select name="teacher_id" id="teacher_id" class="form-control">
              ...
              @foreach($trainers as $trainer)
                  <option value="{{ $trainer->id }}" {{ old('teacher_id', $course->teacher_id) == $trainer->id ? 'selected' : '' }}>
                      {{ $trainer->name }}
                  </option>
              @endforeach
          </select>
          ...
          <select name="trainers[]" id="trainers" class="form-control" multiple>
              @foreach($trainers as $trainer)
                  <option value="{{ $trainer->id }}" 
                      {{ $course->trainers->contains($trainer->id) ? 'selected' : '' }}>
                      {{ $trainer->name }}
                  </option>
              @endforeach
          </select>
